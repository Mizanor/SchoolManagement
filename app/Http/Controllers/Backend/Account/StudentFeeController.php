...
class StudentFeeController extends Controller
{
    ...
    public function getStudent(Request $request){
	    ...
	    foreach ($data as $key => $std) {
	    	...
	    	$accountstudentfees = AccountStudentFee::where('student_id',$std->student_id)->where('class_id',$std->class_id)->where('year_id',$std->year_id)->where('fee_category_id',$fee_category_id)->where('date',$date)->first();
...
public function store(Request $request){
	...
	if(!empty(@$data) || empty($checkdata)){
		return redirect()->route('accounts.fee.view')->with('success','Successfully Updated');
